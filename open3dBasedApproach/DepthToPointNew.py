...
def create_output(vertices, colors,filename): #vertices, colors, filename - parameters
	colors = colors.reshape(-1,3)
	vertices = np.hstack([vertices.reshape(-1,3), colors])
	#vertices = np.hstack([vertices.reshape(-1,3)])
	print("Create Output func executed\n")
	...
	with open(filename, 'w') as f:
		...
		#np.savetxt(f,vertices,'%f %f %f 000 000 000')

output_file = 'pointCloudDeepLearning3.ply'
#Generate point cloud 
create_output(output_points, output_colors,output_file) #output_points, output_colors, output_file
